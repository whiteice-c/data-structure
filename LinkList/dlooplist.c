...
***********作者：baibing
***********时间：2018.2.23	*********
说明：双向循环链表相比于单向循环链表最大的不同就是
多了一个指向前继结点的指针，所以所有的操作都需把前继
结点的指针指向正确的结点即可*/ 

/*1、建立基本结构体：
	包括数据、指向下一个结点的指针、指向前一个结点的指针*/ 
/*基本结构体*/
typedef struct node
{
	int data;
...
void printf_LinkList(LinkNode *head)
{
	...
	if(head == NULL || ((head->next == head) && (head->prior == head)))
	{
...
void insert_to_trail(LinkNode *head,int data)
{
	...
	while(temp1->next != head)/*也可以通过前继结点迅速定位最后一个结点的位置*/
	{
		temp1 = temp1->next;	
	}
...
void delete_same_LinkList(LinkNode *head)
{
	...
	while((rear != head)||(front !=head))/*与单链表不同处*/
	{
		if(front->data == rear->data)
		{
			...
			front->next->prior = temp;	/*删除当前结点而不是靠后的一个，防止找到相同结点并删除靠后的，
										然后继续只遍历之后的导致后面依旧有相同的结点，但无法判断出来*/	
			front = front->next;
